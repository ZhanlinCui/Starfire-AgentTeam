...
**Heartbeat:** Auto-restart on crash, recreate client after 10 failures, proper logging.
**Delegation MCP tools:** Route through platform API (`POST /delegate`, `GET /delegations`) instead of direct peer-to-peer. Full DB tracking + WebSocket events.

## Files Changed (PRs #43-44)
- `workspace-template/cli_executor.py`
- `workspace-template/heartbeat.py` (rewritten)
- `workspace-template/a2a_tools.py` (platform-routed)
- `workspace-template/tests/test_a2a_tools_impl.py`
- `workspace-template/tests/test_heartbeat.py`
