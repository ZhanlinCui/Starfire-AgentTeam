...
## Post-review hardening (/code-review follow-ups)

Three warnings from the post-ecosystem-session code review closed with a
single branch (`fix/code-review-post-ecosystem-session`):

- `platform/internal/handlers/a2a_proxy.go` — `detectPlatformInDocker` now
  uses `strconv.ParseBool` so `STARFIRE_IN_DOCKER=true|false|TRUE` all
  work (previously only the literal `"1"` was accepted). Added
  `setPlatformInDockerForTest(bool) func()` so tests can toggle the
  global without touching the real filesystem.
- `workspace-template/a2a_executor.py` — extracted `DEFAULT_RECURSION_LIMIT
  = 500` constant + `_parse_recursion_limit()` helper that logs a warning
  and falls back on non-integer or non-positive values instead of raising
  `ValueError`.
- Regression tests added:
  - `TestDetectPlatformInDocker_EnvVar` (Go, table-driven)
  - `TestSetPlatformInDockerForTest` (Go)
  - `test_parse_recursion_limit_{default_when_unset,valid_override,
    falls_back_on_garbage,falls_back_on_nonpositive}` (Python)
  - `test_default_recursion_limit_value` (Python, regression guard for the
    `500` ceiling)
  - `TestDeepAgentsAdapter::test_setup_succeeds_with_deepagents_present`
    now asserts `virtual_mode=False` so the bc563d1 fix can't be
    silently reverted.

`.env.example`: `STARFIRE_IN_DOCKER` comment updated to document the
accepted boolean forms.

Test counts after this PR: Python pytest +5, Go handlers +2. Canvas
and SDK unchanged.
