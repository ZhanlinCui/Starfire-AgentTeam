...
## Bugfix — discovery URL rewrite wrong for remote callers

**Caught during the comprehensive Phase 30 live E2E** (after 30.1–30.8
all landed on main). When a remote external caller asked for the URL
of an external target via `/registry/discover/:id`, the handler blindly
rewrote `127.0.0.1` → `host.docker.internal`. That rewrite only makes
sense for Docker-container callers trying to reach the host; remote
callers live outside the Docker network, and `host.docker.internal`
doesn't resolve on their machine.

**Fix** (`platform/internal/handlers/discovery.go`): only apply the
`host.docker.internal` rewrite when the caller's own runtime is
non-external. External callers get the URL unmodified.

**Live verified:** caller=external, target=external with
`url=http://127.0.0.1:9876` now returns `http://127.0.0.1:9876`
unchanged (previously returned `http://host.docker.internal:9876`
and the SDK's `call_peer` hit a dead URL).

All other comprehensive E2E results from the Phase 30 run were
green — this was the only finding.
...
- `local://starfire-dev` → 4612-byte tarball, plugin.yaml + skills/ present
...
**SDK** — `pyproject.toml` rebranded `starfire-sdk@0.2.0` so a single
`pip install starfire-sdk` ships both `starfire_plugin` (plugin
authors) and `starfire_agent` (remote-agent authors). Added trove
classifiers, keywords, requires-python pin. New
`sdk/python/starfire_agent/README.md` quickstart.
