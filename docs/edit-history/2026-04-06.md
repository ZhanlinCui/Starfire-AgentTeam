...
## agent-molecule-status CLI

New `agent_molecule_status.py` — CLI tool + importable module for any process inside a workspace container to update the canvas task display:

```bash
agent-molecule-status "Running weekly audit..."  # show on canvas
agent-molecule-status ""                         # clear
```

Pushes immediate heartbeat (`current_task`) + logs activity (`task_update`). Linked as `/usr/local/bin/agent-molecule-status` in Dockerfile. Importable from Python:
...
- `starfire_http_request_duration_seconds_total{method,path}` — counter (sum)
