...
### DeepAgents Full SDK Utilization + LLM Cache

Upgraded DeepAgents adapter from ~30% to ~95% SDK utilization:
- **FilesystemBackend(/workspace)** — agents can read/write/edit files that persist to disk
- **MemorySaver checkpointer** — session continuity across A2A messages
- **Memory files** — CLAUDE.md loaded natively via DeepAgents MemoryMiddleware
- **Filesystem permissions** — restrict writes to /workspace and /configs only
- **Native skills** — Python files from /configs/skills/*.py loaded as dynamic tools
- **InMemoryCache** — identical prompts return cached responses, saving API cost + latency

Also added InMemoryCache to the LangGraph adapter via `set_llm_cache()`.

### Gemini-Powered Org Template + DeepAgents Support

New org template `starfire-worker-gemini`: 7 agents running on DeepAgents runtime with Google Gemini 2.5 Flash (~20x cheaper than Claude Opus). DeepAgents provides built-in filesystem tools (read/write/edit), task planning (write_todos), sub-agent spawning, shell execution, and auto-summarization — all running on Gemini with no Anthropic key required.

**Root cause fixes discovered during Gemini deployment:**
1. `ensureDefaultConfig` placed model under `runtime_config.model` for non-langgraph runtimes, but `config.py` reads top-level `model`. Every non-langgraph runtime silently fell back to `anthropic:claude-sonnet-4-6`. Fixed: model always at top level.
2. `OrgDefaults` struct was missing `Model` field — org.yaml `defaults.model` didn't propagate to workspaces. Fixed: added field + fallback logic.
3. `claude-code-default` config template had model under `runtime_config.model` instead of top level. Fixed.
4. All 6 adapter Dockerfiles had `USER agent` overriding the gosu entrypoint pattern. Fixed.
5. `chown -R /workspace` recursed the entire bind-mounted repo, hanging PM startup. Fixed: top-level chown only.

**DeepAgents adapter improvements:**
- Added `google_genai` and `ollama` providers to `_create_llm()`
- Pass model to executor for telemetry
- Updated requirements: `deepagents>=0.5.0`, `langchain-google-genai>=2.1.0`, `langchain-anthropic>=1.4.0`
- LangGraph requirements also updated with `langchain-google-genai>=2.1.0`
...
- Example: Security Auditor daily scan in `org-templates/starfire-dev/org.yaml`
