...
- `platform/internal/channels/telegram.go` — Telegram adapter (webhook + long-polling, Markdown retry)
- `platform/internal/channels/manager.go` — orchestrator with hot reload, conversation history (Redis), allowlist, A2A proxy
- `platform/internal/handlers/channels.go` — REST API (CRUD, send, test, webhook)
...
- 29 unit tests (channels package) + 12 handler tests (sqlmock) + 10 E2E API checks
