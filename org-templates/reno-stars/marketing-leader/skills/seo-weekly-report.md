...
After GSC data, collect weekly metrics from all business listing platforms. Use Chrome CDP (port 9222) with puppeteer-core at puppeteer-core. Launch Chrome if needed:
```bash
...
const browser = await puppeteer.connect({ browserURL: "http://host.docker.internal:9223", defaultViewport: null });
