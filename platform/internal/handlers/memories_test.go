...
func TestMemoriesCommit_Local_Success(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO agent_memories").
		WithArgs("ws-1", "The answer is 42", "LOCAL").
...
func TestMemoriesCommit_Global_AsRoot(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO agent_memories").
		WithArgs("root-ws", "global fact", "GLOBAL").
...
func TestMemoriesSearch_LocalScope(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "workspace_id", "content", "scope", "created_at"}).
		AddRow("mem-1", "ws-1", "local memory", "LOCAL", "2024-01-01T00:00:00Z")

	mock.ExpectQuery("SELECT id, workspace_id, content, scope, created_at FROM agent_memories WHERE workspace_id").
		WillReturnRows(rows)
...
func TestMemoriesSearch_GlobalScope(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "workspace_id", "content", "scope", "created_at"}).
		AddRow("mem-g1", "root-ws", "global knowledge", "GLOBAL", "2024-01-01T00:00:00Z")

	mock.ExpectQuery("SELECT id, workspace_id, content, scope, created_at FROM agent_memories WHERE scope = 'GLOBAL'").
		WillReturnRows(rows)
...
func TestMemoriesSearch_TeamScope_AsChild(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "workspace_id", "content", "scope", "created_at"}).
		AddRow("mem-t1", "sibling-ws", "team info", "TEAM", "2024-01-01T00:00:00Z")

	mock.ExpectQuery("SELECT m.id, m.workspace_id, m.content, m.scope, m.created_at").
		WillReturnRows(rows)
